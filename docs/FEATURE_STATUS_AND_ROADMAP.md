...
Current source code is the source of truth. When this document conflicts with the application code, update this document to match the code before using it as a planning reference.
...
- First-pass session persistence restores open photo paths, selected photo, and zoom percent.
- First-pass win-x64 publish script exists under `build/Publish-V1.ps1`.

### Needs Work

- Full project/session persistence beyond the current open-photo restore.
...
- `ShapeBalance` is now first-pass code, not only a future hold item. It uses SnapshotMask, landmarks, and HardProtect as reference data, creates a geometry map, warps image/masks/landmarks together, and then lets skin retouch run on the balanced image and balanced masks when shape controls are active.
- `ShapeBalance` is still not a skin filter. Its geometry map must stay separate from SkinSmooth, BlemishReduce, WrinkleSoftReduce, ToneEven, and TextureRestore.
...
- FaceParsing scaffolding is added: `IFaceParsingDetector`, `FaceParsingInput`, `ParsingMaskSet`, `ParsingLabelMapper`, and `NoFaceParsingDetector`.
- `NoFaceParsingDetector` is the current fallback parsing detector. Real pixel-level FaceParsing AI is not connected yet.
...
- First-pass `PreviewRenderTier` support is in code: `LowPreview`, `FastPreview`, `QualityPreview`, and `ExportRender`. Interactive previews use display-sized sources, while export uses original-resolution rendering.
- `AverageFaceColorMaskBuilder` and `SkinToneMaskBuilder` are added. Average face-color mask preview/debug can be generated from the current SnapshotMask and used as an auto mask support layer.
- `ShapeBalanceProcessor`, `ShapeBalanceMapBuilder`, `FaceSymmetryAnalyzer`, `FeatureCenterLineEstimator`, and ShapeBalance debug export are added. ShapeBalance can cache a balanced image/mask bundle per photo and reuse it until shape controls or source state change.
...
- `DummySnapshotMaskEngine`, `HeuristicPortraitMaskEngine`, and `TemporaryFaceParsingDetector` are no longer present in the current source. The current fallback parsing path is `NoFaceParsingDetector`.
...
- Real pixel-level FaceParsing AI connection for skin, eye, eyebrow, lip, mouth, hair, glasses, beard, and mustache.
- Real portrait visual QA for the current SnapshotMask, ShapeBalance, and skin-retouch pipeline.
...
- NostrilDetector tuning on real portrait failures, especially under-nose shadow and beard-shadow cases.
- HardProtect, SoftProtect, and RetouchAllow visual review on real images.
- Mask quality validation threshold tuning.
- Debug mask view UI polish.
...
- ShapeBalance real portrait QA, especially identity preservation, HardProtect damping, nostril observation, jawline contour, and overshoot behavior.
...
- Shape stage - `1` to `10` ShapeBalance stage preset
- Identity preservation - protects the original impression while applying geometry balance
- Head tilt balance - first-pass roll/pitch-like correction
- Left-right symmetry amount - master symmetry strength with a tiny high-range overshoot zone
- Oval face correction - first ShapeBalance face-outline warp plus older preview support
- Left-right balance - first ShapeBalance manual face-center shift plus older preview support
- Cheekbone soften - first ShapeBalance/manual contour support plus older preview support
...
- Face left-right balance - first ShapeBalance/manual face-center balance
- Eye height balance - first ShapeBalance paired eye-line balance
- Brow height balance - first ShapeBalance paired eyebrow-line balance
- Nose bend center correction - first ShapeBalance central nose-line balance
...
- `ShapeBalanceProcessor` is wired in `MainWindow` and can run before the skin retouch processor when shape controls are active.
- `ShapeBalanceProcessor` takes a `FaceSnapshotMaskSet`, builds a `ShapeBalanceMap`, warps the image, masks, landmarks, and face box together, and returns a balanced bundle.
- `ShapeBalanceMapBuilder` uses SnapshotMask landmarks and feature observations to build global and local warp regions.
- `FaceSymmetryAnalyzer` and `FeatureCenterLineEstimator` provide the first symmetry/centerline analysis.
- ShapeBalance output is cached per photo through `PhotoItem.CachedShapeBalanceBundle` and invalidated when shape controls, manual masks, face work area, or source state change.
...
- Real-image ShapeBalance QA before treating it as finished.
- Individual face keypoint handles for mask alignment.
- Stronger landmark/mask support for pupil size, eye width/height, nostril size/height, and jaw width fields that currently exist as model placeholders.
- Better UI distinction between skin Stage and ShapeBalance Stage.
- Bounds and warnings to prevent unrealistic edits.
- Separate chin/jawline interaction review after real ShapeBalance QA.
